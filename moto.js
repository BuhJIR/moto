 ...
 * Pure Box2D-WASM 7.0.0 implementation based on Liquid.lab patterns
 * Self-contained with all helper methods included
 *
...
(function(global) {
  ...
  // ═══════════════════════════════════════════════════════════════════════
  // BOX2D WORLD HELPER METHODS (from Liquid.lab)
  // These extend b2World.prototype for easy body/joint creation
  // ═══════════════════════════════════════════════════════════════════════
  ...
  function extendWorld(worldInstance) {
     ...
     */
    worldInstance.addFixture = function(obj) {
     ...
     */
    worldInstance.addBodyDef = function(obj) {
      var bd = new b2BodyDef();
      var type = obj.type || 'dynamic';
      bd.position.Set(obj.x || 0, obj.y || 0);
      bd.angle = obj.angle || 0;
      bd.allowSleep = true;
      if (obj.allowSleep !== undefined) bd.allowSleep = obj.allowSleep;
      bd.awake = true;
      if (obj.awake !== undefined) bd.awake = obj.awake;
     ...
     * Create shape
     */
    worldInstance.addShape = function(obj) {
      ...
      switch (shapeName) {
        case 'polygon':
          ...
          for (var i = 0; i < len; i += 2) {
            shape.vertices.push(new b2Vec2(obj.vertices[i] + dx, obj.vertices[i + 1] + dy));
          }
          break;
        case 'box':
          shape = new b2PolygonShape();
    ...
    worldInstance.addJoint = function(obj) {
      ...
      switch (type) {
        case 'wheel':
          joint = jd.InitializeAndCreate(bodyA, bodyB, obj.axis || new b2Vec2(), obj.angle || new b2Vec2());
          break;
        case 'revolute':
          joint = jd.InitializeAndCreate(bodyA, bodyB, obj.axis || new b2Vec2());
          break;
        case 'prismatic':
          joint = jd.InitializeAndCreate(bodyA, bodyB, obj.axis || new b2Vec2(), obj.angle || new b2Vec2());
          break;
        ...
        case 'mouse':
          jd.bodyA = bodyA;
          jd.bodyB = bodyB;
          jd.target = obj.target || bodyB.GetPosition();
          jd.maxForce = obj.maxForce || 1000;
          joint = this.CreateJoint(jd);
          break;
      }
  ...
  Rider.prototype = {
    ...
    createJoint: function(cst, part1, part2) {
      ...
      var axis = new b2Vec2(
        pos.x + this.mirror * cst.axe_position.x,
        pos.y + cst.axe_position.y
      );
    ...
    eject: function() {
      if (!this.moto.isDead) {
        ...
        this.torso.ApplyForce(adjusted, this.torso.GetWorldCenter());
      }
    },
  ...
  Moto.prototype = {
    ...
    update: function(input) {
      ...
      // Acceleration
      if (input.up) {
        this.left_wheel.ApplyTorque(-this.mirror * Constants.moto_acceleration);
      }
